...
function importFlightPlan(state = {}, action) {
  return state;
}
...
function geoSearch(state, action) {
  ...
  switch (action.type) {
    ...
    case c.REQUEST_GEO_SEARCH_RESULTS:
...
function requestGeoSearchAirways(state = {}, action) {
  return Object.assign({}, state, {
    airwaysLoading: true
  });
}

function receiveGeoSearchAirways(state = {}, action) {
  return Object.assign({}, state, {
    airwaysLoading: false
  });
}

function requestGeoSearchResults(state = {}, action) {
  return Object.assign({}, state, {
...
function receiveGeoSearchResults(state = {}, action) {
  return Object.assign({}, state, {
    resultsLoading: false,
    results: action.results
  });
...
function map(state, action) {
  state = state || {
    ...
    overlays: c.MAP_OVERLAYS_DEFAULT
  };
  ...
  switch (action.type) {
    ...
    case c.SET_MAP_OVERLAYS:
      return Object.assign({}, state, {
        overlays: setMapOverlays(state.overlays, action)
      });
  }
...
function setMapBaseLayer(state, action) {
  return action.baseLayer || state || c.MAP_BASE_LAYER_DEFAULT;
}

function setMapOverlays(state, action) {
  return action.overlays || state || c.MAP_OVERLAYS_DEFAULT;
}

function sidebar(state, action) {
  state = state || {
    expanded: false,
    context: c.SIDEBAR_CONTEXT_DEFAULT
  };
...
function setSidebarContext(state, action) {
  return action.context || state || c.SIDEBAR_CONTEXT_DEFAULT;
}
...
// TODO: importFlightPlan must act on root level as it handles waypoints and flightPlans
