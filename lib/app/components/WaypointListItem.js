...
class WaypointListItem extends React.Component {
  ...
  handleDeleteClick() {
    ...
    analytics.trackEvent('WaypointListItem', 'Clicked', 'Delete')();

    function undo() {
      analytics.trackEvent('WaypointListItem', 'Undo', 'Delete')();
  ...
  handleSortUp() {
    const { waypoint, dispatch } = this.props;
    analytics.trackEvent('WaypointListItem', 'Clicked', 'SortUp')();
    dispatch(setWaypointIndex(waypoint.id, waypoint.index - 1.5));
  }

  handleSortDown() {
    const { waypoint, dispatch } = this.props;
    analytics.trackEvent('WaypointListItem', 'Clicked', 'SortDown')();
    dispatch(setWaypointIndex(waypoint.id, waypoint.index + 1.5));
  }

  handleTotalsToggle() {
    analytics.trackEvent('WaypointListItem', 'Clicked', 'TotalsToggle')();
    this.setState({
