...
class WaypointListItem extends React.Component {
  ...
  handleDeleteClick() {
    ...
    analytics.tracker.trackEvent('WaypointListItem', 'Clicked', 'Delete');

    function undo() {
      analytics.tracker.trackEvent('WaypointListItem', 'Undo', 'Delete');
  ...
  handleSortUp() {
    const { waypoint, dispatch } = this.props;
    analytics.tracker.trackEvent('WaypointListItem', 'Clicked', 'SortUp');
    dispatch(setWaypointIndex(waypoint.id, waypoint.index - 1.5));
  }

  handleSortDown() {
    const { waypoint, dispatch } = this.props;
    analytics.tracker.trackEvent('WaypointListItem', 'Clicked', 'SortDown');
    dispatch(setWaypointIndex(waypoint.id, waypoint.index + 1.5));
  }

  handleTotalsToggle() {
    analytics.tracker.trackEvent('WaypointListItem', 'Clicked', 'TotalsToggle');
    this.setState({
