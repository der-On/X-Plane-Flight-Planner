...
import downloadFlightPlan from '../downloadFlightPlan';
...
class FlightPlanSettings extends React.Component {
  ...
  handleColorChange(event, index) {
    const { dispatch, flightPlan } = this.props;
    analytics.tracker.trackEvent('FlightPlanSettings', 'Change', 'Color');
  ...
  handleVisibilityChange(event, visible) {
    const { dispatch, flightPlan } = this.props;
    analytics.tracker.trackEvent('FlightPlanSettings', 'Change', 'Visibility');
  ...
  handleDeleteClick() {
    ...
    if (!deleting) {
      analytics.tracker.trackEvent('FlightPlanSettings', 'Clicked', 'Delete');
      this.setState({
        deleting: true
      });
    } else {
      analytics.tracker.trackEvent('FlightPlanSettings', 'Confirmed', 'Delete');
  ...
  handleDeleteAbortClick() {
    analytics.tracker.trackEvent('FlightPlanSettings', 'Aborted', 'Delete');
    this.setState({
      deleting: false
    });
  }
  ...
  handleExportAs(format) {
    const { dispatch, flightPlan, waypoints } = this.props;
    return (event) => {
      analytics.tracker.trackEvent('FlightPlanSettings', 'Clicked', 'ExportAs', format);
      dispatch(exportFlightPlan(flightPlan.id, format));
      downloadFlightPlan(flightPlan, waypointsForFlightPlan(flightPlan.id, waypoints), format);
    }
  }
  ...
  render() {
    ...
    return (
      <div className="flight-plan-settings">
        ...
        >
          <MenuItem
            primaryText="X-Plane FMS"
            onClick={this.handleExportAs('xplane_fms').bind(this)} />
          <MenuItem
            primaryText="KML"
            onClick={this.handleExportAs('kml').bind(this)} />
          <MenuItem
            primaryText="JSON"
            onClick={this.handleExportAs('json').bind(this)} />
        </IconMenu>
