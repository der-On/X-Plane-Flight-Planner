...
class FlightPlanSettings extends React.Component {
  ...
  handleColorChange(event, index) {
    const { dispatch, flightPlan } = this.props;
    analytics.trackEvent('FlightPlanSettings', 'Change', 'Color')();
  ...
  handleVisibilityChange(event, visible) {
    const { dispatch, flightPlan } = this.props;
    analytics.trackEvent('FlightPlanSettings', 'Change', 'Visibility')();
  ...
  handleDeleteClick() {
    ...
    if (!deleting) {
      analytics.trackEvent('FlightPlanSettings', 'Clicked', 'Delete')();
      this.setState({
        deleting: true
      });
    } else {
      analytics.trackEvent('FlightPlanSettings', 'Confirmed', 'Delete')();
  ...
  handleDeleteAbortClick() {
    analytics.trackEvent('FlightPlanSettings', 'Aborted', 'Delete')();
    this.setState({
      deleting: false
    });
  }
  ...
  handleExportAs(format) {
    ...
    return (event) => {
      analytics.trackEvent('FlightPlanSettings', 'Clicked', 'ExportAs', format);
