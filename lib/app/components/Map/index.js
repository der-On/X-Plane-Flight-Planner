...
class Map extends React.Component {
  ...
  createNavItemMarkers() {
    ...
    navItems.airways
      .forEach((airway) => {
        ...
        if (!group) {
          group = L.layerGroup([
            self.createAirwayMarker(airway, airway.latFrom, airway.lonFrom),
            self.createAirwayMarker(airway, airway.latTo, airway.lonTo),
  ...
  createAirwayLine(airway) {
    var line = L.polyline([
      [airway.latFrom, airway.lonFrom],
      [airway.latTo, airway.lonTo]
    ], {
      dashArray: '5, 5',
      clickable: false,
