...
class Map extends React.Component {
  ...
  bindMapEvents() {
    ...
    function doubleClickHandler(event) {
      const { activeFlightPlanId } = self.props;
      var latLng = event.latlng;
      self.addGpsWaypoint(latLng.lat, latLng.lng);
    }
  ...
  openSidebarIfNoWaypoints() {
    const { dispatch, waypoints, activeFlightPlanId } = this.props;
    if (!waypointsForFlightPlan(activeFlightPlanId, waypoints).length) {
  ...
  createFlightPlans() {
    ...
    function create(id) {
      ...
      // do not create invisible flight plans
      if (!flightPlan.visible) return;
      ...
      } // update latLngs and color
      else {
        marker.setLatLngs(_waypoints.map(navItemLatLng));
        marker.setStyle({
          color: flightPlan.color || FLIGHT_PLAN_COLOR_DEFAULT
        });
      }
    }
  ...
  cleanupFlightPlans() {
    ...
    function filterVisible(id) {
      var flightPlan = flightPlans[id];
      return flightPlan.visible;
    }

    var flightPlanIds = Object.keys(flightPlans).filter(filterVisible);
    var prevFlightPlanIds = Object.keys(self.markers.flightPlans);
  ...
  createWaypoints() {
    ...
    function create(id) {
      ...
      // do not create waypoints for non existing flight plans
      if (!flightPlan) return;

      var latLng = L.latLng(waypoint.lat, waypoint.lon);

      // do not create markers for invisible flight plans
      if (!flightPlan.visible) return;
      ...
      } // update lat lng and color of marker
      else {
        marker.setLatLng(latLng);
        marker.setIcon(Icons.waypoint(flightPlan, waypoint));
      }
    }
  ...
  cleanupWaypoints() {
    ...
    function filterVisible(id) {
      var waypoint = waypoints[id];
      var flightPlan = flightPlans[waypoint.flightPlanId];
      return (flightPlan && flightPlan.visible);
    }
    ...
    var waypointIds = Object.keys(waypoints).filter(filterVisible);
  ...
  handleClick(event) {
    ...
    if (typeof target.dataset.addGpsWaypoint !== 'undefined') {
      this.addGpsWaypoint(parseFloat(target.dataset.lat), parseFloat(target.dataset.lon), parseFloat(target.dataset.elevation));
    }
    if (typeof target.dataset.removeWaypoint !== 'undefined') {
      this.removeWaypoint(target.dataset.id);
    }
  }
  ...
  addGpsWaypoint(lat, lon, elevation) {
    const { dispatch, activeFlightPlanId } = this.props;

    this.openSidebarIfNoWaypoints();
    dispatch(addGpsWaypoint(activeFlightPlanId, lat, lon, elevation));
  }
  ...
  removeWaypoint(id) {
    ...
    dispatch(removeWaypoint(id, undo));
  }
