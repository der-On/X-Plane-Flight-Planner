...
function airwayMarker(airway, lat, lon, label) {
  ...
  if (lat === airway.fromLat && lon === airway.fromLon) {
    popup = Popups.airwayFrom(airway);
  } else {
    popup = Popups.airwayTo(airway);
  }
...
export default {
  ...
  waypoint: (flightPlan, waypoint) => {
    return L.marker(navItemLatLng(waypoint),
      ...
      .bindLabel(padStart((waypoint.index + 1).toString(), 2, '0'), {
        noHide: true
      })
      .bindPopup(Popups.waypoint(waypoint));
